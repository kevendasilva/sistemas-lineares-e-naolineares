...
module Methods
  ...
  def self.gauss(matrix)
    matrix_gauss = Marshal.load(Marshal.dump(matrix)) # Criando uma copiada do parãmetro de entrada
  ...
  def self.err_x_i(initial_solution, next_solution)
    ...
    num_of_interations.times do |i| # Verificando o erro relativo estimado
      if ((next_solution[i] - initial_solution[i]) / initial_solution[i]).abs < 0.000001
  ...
  def self.jacobi(initial_solution, matrix, ite)
    ...
    if err_x_i.include? false 
      jacobi(next_solution, matrix, ite) # Uma nova iteração é iniciada.
  ...
  def self.gauss_seidel(initial_solution, matrix, ite)
    ...
    # Caso o erro de uma das variáveis não atenda a condição estabelecida.
    if err_x_i.include? false
      gauss_seidel(next_solution, matrix, ite) # Uma nova iteração é iniciada.
    # Caso a solução encontrada seja satisfatória
    else
